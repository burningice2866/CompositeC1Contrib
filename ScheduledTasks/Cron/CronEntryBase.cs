...
{
    public abstract class CronScheduleBase: ICronEntry
    {
        ...
        private void ParseEntry(string entry)
        {
            ...
            {
                throw new ArgumentNullException("Entry is empty.");
            }
            ...
            if (entry.IndexOf("/") != -1)
            {
                ...
                {
                    throw new Exception("Found unexpected character. entry=" + entry);
                }
                if (interval < 1)
                {
                    throw new Exception("Interval out of bounds.");
                }
            }
            ...
            else
            {
                ...
                if (entry.IndexOf("-") != -1)
                {
                    ...
                    if (minUsed < MinValue)
                    {
                        throw new Exception("Minimum value less than expected.");
                    }
                    ...
                    if (maxUsed > MaxValue)
                    {
                        throw new Exception("Maximum value greater than expected.");
                    }
                    if (minUsed > maxUsed)
                    {
                        throw new Exception("Maximum value less than minimum value.");
                    }
                    AddValues(minUsed, maxUsed, interval);
                }
                else
                {
                    ...
                    if (minUsed < MinValue)
                    {
                        throw new Exception("Value is less than minimum expected.");
                    }
                    if (interval == -1)
                    {
                        ...
                        if (maxUsed > MaxValue)
                        {
                            throw new Exception("Value is greater than maximum expected.");
                        }
                        AddValues(minUsed, maxUsed, interval);
                    }
                    else
